...
def main():
    ...
    if len(sys.argv) != 3:
        print("Uso: PY-MP-008-22 ExtraccionMaterial.py <archivo_pdf> <archivo_excel>")
        sys.exit(1)
    ...
    imagenes = convert_from_path(archivo_pdf,poppler_path=r'D:\03. PROFESSIONAL EXP\02. MARCO PERUANA\UIPath+Python\PY-MP-008 DigitalizacionInicialLM\poppler-24.02.0\Library\bin', dpi=300)

    # Inicializar variable Material
    Material = ""
    ...
    for imagen in imagenes:
        ...
        if width > height:
            izquierda = int(0.28 * width)
            top = int(0.08 * height)
            derecha = int(0.35 * width)
            bottom = int(0.9 * height)

        elif height > width:
            izquierda = int(0.55 * width)
            top = int(0.08 * height)
            derecha = int(0.714 * width)
        ...
        custom_config = r'--dpi 300 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ,.-& '
        texto_extraido = pytesseract.image_to_string(imagen_preprocesada, config=custom_config)

        # Concatenar el texto extraído a Material
        Material += texto_extraido + '\n'

    # Imprimir Material
    Material = Material.replace("\n", "|")
    print(Material)

    # Generar la lista de palabras ListaMaterial
    ListaMaterial = Material.split("||")

    # Cargar el archivo Excel
    workbook = openpyxl.load_workbook(archivo_excel)
    sheet = workbook.active

    # Insertar las palabras en el Excel de forma vertical a partir de la celda D3
    fila_inicial = 3
    columna = 4  # Columna D

    for index, palabra in enumerate(ListaMaterial):
