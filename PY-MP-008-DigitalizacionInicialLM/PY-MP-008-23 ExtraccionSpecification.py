...
def main():
    ...
    if len(sys.argv) != 3:
        print("Uso: PY-MP-008-23 ExtraccionSpecification.py <archivo_pdf> <archivo_excel>")
        sys.exit(1)
    ...
    # Inicializar variable Specification
    Specification = ""
    ...
    for imagen in imagenes:
        ...
        if width > height:
            izquierda = int(0.36 * width)
            top = int(0.08 * height)
            derecha = int(0.51 * width)
            bottom = int(0.9 * height)

        elif height > width:
            izquierda = int(0.682 * width)
            top = int(0.08 * height)
            derecha = int(1.0 * width)
        ...
        custom_config = r'--dpi 300 -c tessedit_char_whitelist= ABCDEFGHIJKLMNOPQRSTUVWXYZx,.-/0123456789 '
        texto_extraido = pytesseract.image_to_string(imagen_preprocesada, config=custom_config)

        # Concatenar el texto extraído a Specification
        Specification += texto_extraido + '\n'

    # Imprimir Specification
    Specification = Specification.replace("\n", "|")
    print(Specification)

    # Generar la lista de palabras ListaSpecification
    ListaSpecification = Specification.split("||")

    # Cargar el archivo Excel
    workbook = openpyxl.load_workbook(archivo_excel)
    sheet = workbook.active

    # Insertar las palabras en el Excel de forma vertical a partir de la celda E3
    fila_inicial = 3
    columna = 5  # Columna E

    for index, palabra in enumerate(ListaSpecification):
